...
extern "C" void app_main()
{
    ...
    light_config.color_control_color_temperature.color_temperature_mireds = mireds_from_ct((WW_CT + WC_CT)/2);
    light_config.color_control_color_temperature.color_temp_physical_min_mireds = mireds_from_ct(WC_CT);
    light_config.color_control_color_temperature.color_temp_physical_max_mireds = mireds_from_ct(WW_CT);

    // endpoint handles can be used to add/modify clusters.
    endpoint_t *endpoint = color_temperature_light::create(node, &light_config, ENDPOINT_FLAG_NONE, light_handle);
    ABORT_APP_ON_FAILURE(endpoint != nullptr, ESP_LOGE(TAG, "Failed to create ct light endpoint"));
